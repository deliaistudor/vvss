...
public class OrdersGUIController {
    ...
    private ObservableList<OrderItem> menuData;// = FXCollections.observableArrayList();
    ...
    public void initialize(){
        ...
        tableMenuItem.setCellValueFactory(
                new PropertyValueFactory<OrderItem, String>("menuItem"));
        tablePrice.setCellValueFactory(
                new PropertyValueFactory<OrderItem, Double>("price"));
        tableQuantity.setCellValueFactory(
                new PropertyValueFactory<OrderItem, Integer>("quantity"));

        //bind pizzaTypeLabel and quantity combo box with the selection on the table view
        orderTable.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<OrderItem>() {
        @Override
